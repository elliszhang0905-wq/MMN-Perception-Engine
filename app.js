...
const SUMMARY_ATTRIBUTE_CLUSTER_THRESHOLD=9;
let summaryAttributeActiveLabel="",summaryAttributeActiveCategory="全部",summaryAttributeExpandedQuadrant="",summaryAttributeEvidenceExpanded=false;
...
function socialCompetitorCatalog(models,own,selected){
 const excluded=new Set([own,...selected].filter(Boolean));
 return brandModelGroups(models.filter(model=>model&&!excluded.has(model))).map(group=>({brand:group.brand,models:group.models.map(model=>({value:model,label:modelNameUnderBrand(group.brand,model)}))}));
}
function renderSocialCompetitorPicker(){
 ...
 const own=document.querySelector("#social-trend-keyword")?.value.trim()||state.config.model||"",previousBrand=brandSelect.value;
 ...
 modelSelect.onchange=()=>{const value=modelSelect.value;if(value&&socialTrendState.competitors.length<3&&!socialTrendState.competitors.includes(value))socialTrendState.competitors.push(value);renderSocialCompetitorPicker()};
...
function renderSocialTrends(){
 ...
 const riskCard=box.querySelector(".social-kpi-grid article.risk");if(riskCard&&riskItems.length){riskCard.classList.add("social-risk-trigger");riskCard.setAttribute("role","button");riskCard.setAttribute("tabindex","0");riskCard.setAttribute("aria-haspopup","dialog");riskCard.setAttribute("aria-expanded","false");const hint=riskCard.querySelector("small");if(hint)hint.textContent="点击查看具体内容";riskCard.insertAdjacentHTML("beforeend",riskPopover)}
...
async function importSocialTrendFile(file){
 const keyword=document.querySelector("#social-trend-keyword")?.value.trim(),platforms=[...document.querySelectorAll('[name="social-platform"]:checked')].map(x=>x.value),thresholds=socialThresholdValues();if(!keyword)throw new Error("请先填写车型关键词");
...
function renderSummaryAttributeOpportunityBoard(rows,models,source){
 ...
 const active=items.find(item=>item.label===summaryAttributeActiveLabel)||visibleItems[0]||items[0],sources=state.importQuality?.attributeNsrSources||[...new Set(rows.map(row=>row[2]).filter(Boolean))],scoreFor=(model,platform)=>meanNumbers(rows.filter(row=>row[0]===model&&row[2]===platform&&row[4]===active.label).map(row=>nullableNumber(row[14])).filter(value=>value!==null)),sourceRows=sources.map(platform=>{const own=scoreFor(ownModel,platform),competitor=meanNumbers(competitors.map(model=>scoreFor(model,platform)).filter(value=>value!==null));return{platform,own,competitor,gap:own!==null&&competitor!==null?own-competitor:null}}),gapText=active.gap===null?"—":`${active.gap>=0?"领先":"落后"} ${Math.abs(active.gap*100).toFixed(1)}%`;
 const quadrants=[
  {key:"weak",title:"认知待建立",note:"NSR 负向 · 相对领先",filter:item=>item.ownNsr<0&&(item.gap===null||item.gap>=0)},
  {key:"strength",title:"领先优势",note:"NSR 正向 · 领先竞品",filter:item=>item.ownNsr>=0&&(item.gap??0)>=0},
  {key:"repair",title:"双重风险",note:"NSR 负向 · 落后竞品",filter:item=>item.ownNsr<0&&item.gap!==null&&item.gap<0},
  {key:"pressure",title:"对标短板",note:"NSR 正向 · 落后竞品",filter:item=>item.ownNsr>=0&&item.gap!==null&&item.gap<0}
 ];
 const bubbleMarkup=(item,quadrantKey,extra="",className="")=>{const gap=item.gap===null?"待补对标":`${item.gap>=0?"领先":"落后"}${Math.abs(item.gap*100).toFixed(1)}%`,size=Math.round(60+Math.max(1,Math.min(5,item.impact))*4);return`<button type="button" class="summary-quadrant-bubble ${summaryAttributeCategoryTone(item.category,item.label)} ${item.label===active.label?"active":""} ${className}" data-summary-attribute-item="${escapeAttr(item.label)}" data-summary-attribute-quadrant="${escapeAttr(quadrantKey)}" ${extra} aria-pressed="${item.label===active.label?"true":"false"}" aria-label="下钻${escapeAttr(item.label)}，本品NSR ${summaryNsrDisplay(item.ownNsr)}，${escapeAttr(gap)}" style="--bubble-size:${size}px"><span>${escapeHtml(item.label)}</span><small>NSR ${summaryNsrDisplay(item.ownNsr)}</small><em class="${item.gap!==null&&item.gap<0?"risk":""}">${escapeHtml(gap)}</em></button>`};
 const clusterMarkup=(quadrantItems,quadrantKey)=>{const center=quadrantItems[0],satellites=quadrantItems.slice(1);return`<div class="summary-quadrant-cluster" data-summary-cluster="${escapeAttr(quadrantKey)}">${satellites.map((item,index)=>{const angle=index/Math.max(satellites.length,1)*Math.PI*2,radius=18+(index%3)*5,x=Math.round(Math.cos(angle)*radius),y=Math.round(Math.sin(angle)*radius),size=Math.round(60+Math.max(1,Math.min(5,item.impact))*4);return`<span class="summary-quadrant-cluster-orb ${summaryAttributeCategoryTone(item.category,item.label)}" style="--bubble-size:${size}px;--cluster-x:${x}px;--cluster-y:${y}px;--cluster-order:${index}" title="${escapeAttr(item.label)}" aria-hidden="true"></span>`}).join("")}${bubbleMarkup(center,quadrantKey,`data-summary-cluster-open="${escapeAttr(quadrantKey)}"`,"cluster-center")}<span class="summary-quadrant-cluster-hint"><b>${quadrantItems.length}</b> 个标签 · 点击中心散开</span></div>`};
 if(summaryAttributeExpandedQuadrant){const expandedDefinition=quadrants.find(quadrant=>quadrant.key===summaryAttributeExpandedQuadrant),expandedCount=expandedDefinition?visibleItems.filter(expandedDefinition.filter).length:0;if(expandedCount<=SUMMARY_ATTRIBUTE_CLUSTER_THRESHOLD)summaryAttributeExpandedQuadrant=""}
 const quadrantMarkup=quadrants.map(quadrant=>{const quadrantItems=visibleItems.filter(quadrant.filter),clustered=quadrantItems.length>SUMMARY_ATTRIBUTE_CLUSTER_THRESHOLD,expanded=clustered&&summaryAttributeExpandedQuadrant===quadrant.key,content=!quadrantItems.length?`<p>暂无标签</p>`:clustered&&!expanded?clusterMarkup(quadrantItems,quadrant.key):quadrantItems.map(item=>bubbleMarkup(item,quadrant.key)).join("");return`<section class="summary-quadrant-cell ${quadrant.key} ${clustered?"has-cluster":""} ${expanded?"cluster-expanded":""}"><header><div><b>${quadrant.title}</b><small>${quadrant.note}</small></div><span class="summary-quadrant-count"><strong>${quadrantItems.length}</strong>${expanded?`<button type="button" data-summary-cluster-close="${escapeAttr(quadrant.key)}">收起聚合</button>`:""}</span></header><div class="summary-quadrant-cloud ${clustered&&!expanded?"clustered":""}">${content}</div></section>`}).join("");
...
function renderSummaryHeatDashboard(a){
 ...
 const ownModelSelect=document.querySelector("#summary-own-model");if(ownModelSelect)ownModelSelect.onchange=event=>{const nextModel=event.target.value;summaryAttributeActiveLabel="";summaryAttributeActiveCategory="全部";summaryAttributeExpandedQuadrant="";summaryAttributeEvidenceExpanded=false;summaryDashboardModels=[nextModel,...available.filter(model=>model!==nextModel).slice(0,3)];applyModelSelection(nextModel);save();render()};
 ...
 document.querySelectorAll("[data-summary-attribute-item]").forEach(item=>item.onclick=()=>{if(item.dataset.summaryClusterOpen)return;const quadrant=item.dataset.summaryAttributeQuadrant;if(summaryAttributeExpandedQuadrant&&quadrant!==summaryAttributeExpandedQuadrant)summaryAttributeExpandedQuadrant="";summaryAttributeActiveLabel=item.dataset.summaryAttributeItem;summaryAttributeEvidenceExpanded=false;renderSummaryHeatDashboard(a)});
 document.querySelectorAll("[data-summary-cluster-open]").forEach(item=>item.onclick=()=>{summaryAttributeExpandedQuadrant=item.dataset.summaryClusterOpen;summaryAttributeActiveLabel=item.dataset.summaryAttributeItem;summaryAttributeEvidenceExpanded=false;renderSummaryHeatDashboard(a)});
 document.querySelectorAll("[data-summary-cluster-close]").forEach(button=>button.onclick=()=>{summaryAttributeExpandedQuadrant="";renderSummaryHeatDashboard(a)});
 document.querySelectorAll("[data-summary-attribute-category]").forEach(category=>category.onclick=()=>{summaryAttributeActiveCategory=category.dataset.summaryAttributeCategory;summaryAttributeActiveLabel="";summaryAttributeExpandedQuadrant="";summaryAttributeEvidenceExpanded=false;renderSummaryHeatDashboard(a)});
 ...
 const attributePlatform=document.querySelector("#summary-attribute-platform");if(attributePlatform)attributePlatform.onchange=event=>{summaryAttributePlatform=event.target.value;summaryAttributeActiveLabel="";summaryAttributeExpandedQuadrant="";summaryAttributeEvidenceExpanded=false;renderSummaryHeatDashboard(a)};
}
...
function largeTrend(rows,allPeriods=[]){
 ...
 return`<svg viewBox="0 0 ${w} ${h}">${[1,Math.ceil(maxRank/2),maxRank].map(r=>`<line x1="${pad}" y1="${pad+(r-1)/(maxRank-1)*(h-pad*2)}" x2="${w-pad}" y2="${pad+(r-1)/(maxRank-1)*(h-pad*2)}"></line><text x="10" y="${pad+(r-1)/(maxRank-1)*(h-pad*2)+4}">${r}</text>`).join("")}${trendPolylineMarkup(pos,"pos-line")}${trendPolylineMarkup(neg,"neg-line")}${pos.map((p,index)=>p?`<circle class="pos-dot" cx="${p.x}" cy="${p.y}" r="5"></circle><text class="point-label" x="${p.x}" y="${trendPointLabelY(p,neg[index],"pos")}" text-anchor="middle">${p.rank}</text>`:"").join("")}${neg.map((p,index)=>p?`<circle class="neg-dot" cx="${p.x}" cy="${p.y}" r="5"></circle><text class="point-label" x="${p.x}" y="${trendPointLabelY(p,pos[index],"neg")}" text-anchor="middle">${p.rank}</text>`:"").join("")}${periods.map((p,i)=>`<text class="x-label ${present.has(p)?"":"missing"}" x="${pad+i/den*(w-pad*2)}" y="${h-10}" text-anchor="middle">${p}</text>`).join("")}</svg><div class="trend-legend"><span><i class="green"></i>正向排名</span><span><i class="red"></i>反向排名</span></div>`;
}
...
function updateBrandPenetrationProgress(job){
 ...
 if(status)status.textContent=job?.status==="completed"?"分析完成":job?.status==="failed"?"分析失败":`正在分析 ${progress}%`;
...
async function runBrandPenetrationProject(config){
 ...
 if(isOfficial){if(status)status.textContent="正在刷新真实快照";await loadBrandPenetrationSnapshot();return}
 try{
  updateBrandPenetrationProgress({progress:0,status:"queued",stage:"queued",message:"采集任务正在排队"});
  const started=await api("/api/social-trends/jobs",{method:"POST",body:JSON.stringify({keyword:config.ownBrand,competitors:(config.competitors||[]).slice(0,5),platforms:["douyin","xiaohongshu","weibo"],timeRange:`${config.range||30}d`,edition:activeEdition()})});
  let job=started.job;
