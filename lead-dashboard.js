(function(){
 ...
 function renderEmpty(model){
  ...
  root.innerHTML=`<section class="lead-dashboard-card empty" aria-labelledby="lead-dashboard-title"><header><div><span>线索看板 · 跟随销量预警车型</span><h2 id="lead-dashboard-title">${esc(name)} 线索表现</h2><p>车型由上方销量预警统一选择。</p></div><em>数据待接入</em></header><div class="lead-dashboard-empty" role="status"><b>${esc(name)}线索数据待接入</b><p>已清空上一车型数据，避免跨车型误读。接入阶段目标、实际线索与实际订单后自动生成看板。</p></div><footer><span>当前车型上下文：${esc(name)}</span><small>未取得平台来源、内容ID与内容标签时，不生成内容归因结论。</small></footer></section>`;
 }
	 ...
	 function render(){
	  ...
	  const stageGroups=data.phases.map((phase,index)=>{
	   const isCurrent=index===data.phases.length-1,isDivergent=phase.leadRate>=1&&phase.orderRate<.8;
