 ...
 function renderEmpty(model){
  ...
  root.innerHTML=`<section class="lead-dashboard-card empty" aria-labelledby="lead-dashboard-title"><header><div><span>线索看板 · 跟随销量预警车型</span><h2 id="lead-dashboard-title">${esc(name)} 线索表现</h2><p>车型由上方销量预警统一选择。</p></div><em>数据待接入</em></header><div class="lead-dashboard-empty" role="status"><b>${esc(name)}线索数据待接入</b><p>已清空上一车型数据，避免跨车型误读。接入阶段目标、实际线索与实际订单后自动生成看板。</p></div><footer><span>当前车型上下文：${esc(name)}</span><small>未取得平台来源、内容ID与内容标签时，不生成内容归因结论。</small></footer></section>`;
	 ...
	 function render(){
	  ...
	  root.innerHTML=`<section class="lead-dashboard-card" aria-labelledby="lead-dashboard-title"><header><div><span>线索看板 · 跟随销量预警车型</span><h2 id="lead-dashboard-title">${esc(activeModel)} 线索表现</h2><p>车型由上方销量预警统一选择。</p></div><em>销量预警 → 线索 → 全局车型分析</em></header><div class="lead-dashboard-summary"><article><span>当前销量预警</span><strong class="${esc(warning.level||"yellow")}">${esc(warning.label||"待复核")} · ${number(warning.sales)} 辆</strong><small>头部基准达成率 ${percent(warning.performanceRate)} · ${esc(warning.cycle||"周期待核验")}</small></article>${summaryPhases.map((phase,index)=>`<article><span>${index===2?"平销进行中":esc(phase.name)}</span><strong class="${rateClass(phase.leadRate)}">线索 ${percent(phase.leadRate)}</strong><small>订单达成 ${percent(phase.orderRate)}</small></article>`).join("")}</div><div class="lead-dashboard-phases">${phases}</div><div class="lead-dashboard-diagnosis"><article><span>阶段异常</span><b>线索超目标，订单未同步增长</b><p>6月下半月线索达成117.7%，订单仅64.6%；需要继续核查内容来源与认知阻力。</p></article>${attributionCard}<article><span>全局联动</span><b>驾驶舱分析对象：${esc(activeModel)}</b><p>现有T周期、正反向、NSR和策略模块统一读取同一车型上下文。</p></article></div><footer><span>来源：${esc(data.source.label)} · ${esc(data.source.scope)}</span><small>缺失字段：平台来源、内容ID、内容标签、线索订单关联</small></footer></section>`;
	 }
