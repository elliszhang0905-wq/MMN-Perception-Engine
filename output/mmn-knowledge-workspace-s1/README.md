...
> 后续发布更新：用户另行授权推送与服务器变更。原型运行提交 `771abe3` 已推送独立分支并部署 ECS 独立容器；服务器版当前经 SSH 隧道访问 `http://127.0.0.1:18792/`。本节之后保留 S1 制作时点记录；最新部署身份与验证见 `docs/研发档案/2026-09-08_s1-20260908-1_知识工作区独立预览发布.md`（仓库根目录相对路径）。不改变 S2/S3 未开始、真实能力未接通的边界。
