(() => {
  ...
  function currentItems() {
    return state.run?.result?.lists?.[state.tab] || [];
  }
  ...
  function results() {
    ...
    if (!run) return `<div class="dvr-empty">
      <b>按车型建立专属抖音内容榜</b>
      <p>点击“开始抓取”，系统会按本品、竞品、对比关系和产品属性建立 7 / 14 / 30 天榜单。不会自动运行。</p>
    </div>`;
    ...
    return `<div class="dvr-summary">
      ...
      <div><span>待人工复核</span><b>${Number(counts.pendingReview) || 0}</b></div>
      <div><span>榜单周期</span><b>近 ${Number(run.rangeDays)} 天</b></div>
    </div>
    ${run.status === "partial" ? `<div class="dvr-running"><b>部分内容未取得可核验播放量</b><p>这些内容不会进入正式热榜，也不会显示为 0 播放。</p><button type="button" class="primary" data-dvr-retry>仅重试热度补抓</button></div>` : ""}
    ...
    ${items.length ? `<ol class="dvr-list">${items.map(resultRow).join("")}</ol>` : `<div class="dvr-empty small"><b>当前分组暂无已核验内容</b><p>这不代表周期内没有相关内容，只代表本轮返回证据未通过该分组门槛。</p></div>`}
    ${strategyPanel()}`;
  }
  function render() {
    ...
    mount.innerHTML = `<article class="panel dvr-panel">
      <header class="dvr-head">
        <div><span>DOUYIN VEHICLE RADAR</span><h2 id="douyin-vehicle-radar-title">本竞品抖音内容雷达</h2><p>围绕当前产品评价数据，手动抓取本品与竞品的热门内容并形成可追溯策略。</p></div>
      ...
      <div class="dvr-controls">
        <div class="dvr-range" aria-label="选择抓取时间范围">${[7, 14, 30].map(days =>
          `<button type="button" data-dvr-range="${days}" class="${state.rangeDays === days ? "active" : ""}" ${state.loading || activeStatuses.has(state.run?.status) ? "disabled" : ""}>${days}天</button>`
        ).join("")}</div>
        <button type="button" class="primary dvr-run" data-dvr-run ${!context.available || state.loading || activeStatuses.has(state.run?.status) ? "disabled" : ""}>${activeStatuses.has(state.run?.status) ? "抓取进行中…" : "开始抓取"}</button>
      </div>
  ...
  function bind() {
    mount.querySelectorAll("[data-dvr-range]").forEach(button => button.onclick = () => {
      ...
      state.run = null; state.strategy = null; state.error = ""; render();
    });
    ...
    try {
      const payload = await request("/api/douyin-vehicle-radar/runs", {
        ...
        body: JSON.stringify({
          ...
          competitors: state.context.competitors || [],
  ...
  async function loadLatest() {
    if (!state.context.available || !state.context.subject) return;
    try {
      const payload = await request(`/api/douyin-vehicle-radar/latest?edition=${encodeURIComponent(edition())}&subject=${encodeURIComponent(state.context.subject)}`);
      state.run = payload.run;
      state.strategy = payload.strategy;
    ...
    try {
      const payload = await request("/api/douyin-vehicle-radar/items/review", {
        ...
        body: JSON.stringify({
          ...
          model: state.context.subject, reason: reason.trim(),
        }),
      });
