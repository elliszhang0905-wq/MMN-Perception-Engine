...
- E7X 数据核查：2026 年 6 月产品评价原始资产仍在，包含全网声量 235,579、互动 2,169,813、整体 NSR 0.7512874631 和 15 个属性；页面“平台情绪标签四象限”显示 0，是因为车型选择只切换分析对象，当前行数据仍为小米 SU7 六车数据集，筛选不到 E7X，不是数据库删除。本次未做数据恢复或覆盖。
- 数据保护：未重建、清空或覆盖 `data/commercial_demo.db`；月度同步兼容性使用临时数据库验证。当前数据库与 2026-07-18、2026-07-19 两份备份中的项目快照均无 E7X 逐条产品评价行，快照状态一致。
- 本地验证：完整 Python 测试 `424/424` 通过；发布门禁 Python 专项 `144/144` 通过；JavaScript、数据优先浏览器检查、桌面与 390 像素全表面检查通过。覆盖 19 个客户页面、8 个管理视图，`failed: []`、`runtimeErrors: []`、`failedResponses: []`，整页横向溢出为 0。
