...
- 交付状态：本地发布候选已完成，等待负责人最终确认与推送；本条不代表已部署 ECS。
- 建议版本码：`beta-1.02-20260720-cloud-demo-consistency-1`。
...
- E7X 数据联动修复：确认 2026 年 6 月产品评价原始资产未丢失；选择 E7X 后现同步绑定奥迪 E7X、小米 YU7、Model Y、问界 M7、奥迪 Q6L e-tron 五车数据集、15 个 E7X 属性、四车属性均值口径、竞品配置及企业卖点车型，不再残留小米 SU7 六车数据。本次没有恢复、覆盖或伪造逐竞品属性数据。
- 数据保护：未重建、清空或覆盖 `data/commercial_demo.db`；月度同步兼容性使用临时数据库验证。当前数据库与 2026-07-18、2026-07-19 两份备份中的项目快照均无 E7X 逐条产品评价行，快照状态一致。
- 本地验证：完整 Python 测试 `424/424` 通过；发布门禁 Python 专项 `144/144` 通过；JavaScript、数据优先浏览器检查、桌面与 390 像素全表面检查通过。覆盖 19 个客户页面、8 个管理视图，并真实点击 E7X 后断言五车数据集、15 个属性、四车均值、竞品配置及卖点车型一致；`failed: []`、`runtimeErrors: []`、`failedResponses: []`，整页横向溢出为 0。
- 发布前剩余动作：负责人确认提交内容后推送 GitHub；随后按既有 ECS 归档发布流程先备份持久化数据，再部署并使用真实试用账号验证分析白名单、使用管理员账号验证资产管理，以及复核月度任务运行报告。本地门禁通过不替代这三项生产验收。
