...
- 建议版本码：`beta-1.02-20260720-cloud-demo-consistency-3`。
...
- 本地验证：完整 Python 测试 `427/427` 通过；发布门禁 Python 专项 `144/144` 通过；JavaScript、数据优先浏览器检查、桌面与 390 像素全表面检查通过。覆盖 19 个客户页面、8 个管理视图，并真实点击 E7X 后断言五车数据集、207 条记录、15 个属性、3 类属性来源、9 个分平台声量字段、7 个平台 NSR、竞品配置及卖点车型一致；`failed: []`、`runtimeErrors: []`、`failedResponses: []`，整页横向溢出为 0。
