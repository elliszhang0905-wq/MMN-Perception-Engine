...
## 2026-07-24｜beta 1.03 抖音单视频创作转化（本地发布）

- 最终版本码：`beta-1.03-20260724-douyin-video-creation-1`；功能提交 `f84cadb` 已推送 GitHub 隔离分支 `codex/douyin-video-creation-20260724`，并从该提交部署本地 8765。不覆盖原始脏工作树，不部署 ECS。
- 抖音排行榜中的已验证视频可进入创作简报，按品牌、车型、目标人群、传播任务、利益点和风格生成且仅生成三条原创方向；每条方向保留当前视频证据引用，并明确禁止照搬原文案、人物身份和镜头组合。
- 新增“爆款内容拆解库”，支持粘贴具体抖音视频链接、搜索、状态筛选、收藏、最多三项结构对比，以及把选定方向和来源上下文交给现有达人方法生成完整脚本；未选择达人资产时脚本按钮保持禁用。
- 创作资产按组织、版本、洞察、创作简报指纹与提示词/结构版本持久化和幂等复用；证据不足、洞察未验证或三路未形成可发布结论时拒绝生成，不把视频热度推断为产品或销量因果。
- 验证基线：创作方向与逐视频洞察专项 `25/25`、抖音榜单/内容防线/脚本生成扩大回归 `73/73`、状态门禁和数据优先 UI 合同通过；真实浏览器已覆盖排行榜到三方向、缓存命中、拆解库、收藏、搜索空态、非法外链、1440px 与 390px。
- 完整发布门禁在后端 189 项阶段出现 14 项环境错误，原因是隔离工作树没有品牌穿透基准快照表且未配置外部社媒服务密钥；没有复制业务数据或注入密钥来制造通过。本功能相关用例无失败，外部模型真实脚本文本质量尚未验收。
- 本地部署后健康接口返回本版本码；真实页面在1440px与390px确认“榜单发现”和“爆款内容拆解库”可达，创作资产列表接口为200、控制台错误为0。验收只读页面触发既有车型身份懒刷新，涉及的5条记录已按主键从部署前备份精确恢复。
- 数据保护：部署前业务库备份为 `backups/mmn_local_pre_douyin_video_creation_20260724_225756.db`（SHA-256 `d8deeb15956adfb9768fb2dd7939b5f241bb4aefa6437b4c6ec1e86a83b8733d`）；部署后原77张表逐表逻辑内容不变，仅新增0行的 `douyin_video_creation_plans` 表，`quick_check=ok`。
