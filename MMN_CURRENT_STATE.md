...
- 状态包版本：`v2.36`
- 最后核验时间：`2026-07-30`（Asia/Shanghai，决策驾驶舱车型内容热度证据发布候选）
- Git应用基线：应用提交 `1029256a23649e07d54c42c5a5e747a009caf537` 已同步功能分支、GitHub `main`、标签 `beta-1.03-20260729-full-deployment-closure-1`、本地8765与ECS；服务器 `.deployed-git-commit`、健康版本和核心文件哈希一致。GitHub `main` 可在本状态包提交后仅领先纯文档闭环，不代表应用代码漂移。
- 当前分支：生产仍运行 `1029256a23649e07d54c42c5a5e747a009caf537`；本轮候选位于隔离分支 `codex/mmn-dashboard-dy-rank-20260730`，未通过主线同步、生产备份、真实TikHub运行和双端验收前不得写成已发布。
...
- 2026-07-30 决策驾驶舱车型内容热度证据候选：在 `03 竞品认知` 内、竞品趋势之后和卖点决策之前提供独立车型输入、7/14/30天与Top 10/20/50。单车型模式不修改驾驶舱本品/竞品配置，服务端强制清空竞品与属性词；搜索按游标受控分页，默认上限为每查询10页、全轮30次请求和300条候选。车型关联或播放量未核验的内容不进入正式榜，缺失播放量不补0；正式榜按播放量、发布时间和平台内容标识稳定排序。达到页数、请求或候选上限时只标记部分覆盖，不宣称平台全库。相关Python回归94项、JavaScript合同、语法与差异检查已通过；1440px和390px本地页面无横向溢出且控制台错误为0。健康接口的版本码与发布日期统一为本轮 2026-07-30。当前仍是发布候选，生产真实运行、数据对比和部署闭环待完成。
