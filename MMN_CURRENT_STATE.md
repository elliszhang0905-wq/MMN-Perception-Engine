...
- 状态包版本：`v2.41`
- 最后核验时间：`2026-07-30`（Asia/Shanghai，抖音逐视频证据链恢复发布候选）
- Git应用基线：政策自动审核生产闭环提交 `c8839a3` 已纳入当前基线；抖音逐视频证据链恢复发布候选版本为 `beta-1.03-20260730-douyin-video-evidence-chain-2`，GitHub提交、ECS标记与最终哈希须在生产收口后补记。
- 当前分支：发布源位于隔离分支 `fix/douyin-video-evidence-chain-20260730`；现有LS6脏工作树及其数据库不纳入发布归档。
