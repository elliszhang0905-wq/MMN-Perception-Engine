...
- 状态包版本：`v2.49`
- 最后核验时间：`2026-07-31`（Asia/Shanghai，LS6管理看板与认知星图发布候选）
- Git应用基线：当前生产仍运行 `beta-1.03-20260731-policy-demo-ready-1`；本次发布候选已将应用与静态资源版本更新为 `beta-1.03-20260731-ls6-cognition-quadrant-1`，尚需完整门禁、主线合并与ECS验收后才能替代生产基线。
- 当前分支：生产基线为 GitHub `main`；本次仅把已审计的 LS6 管理看板与认知星图增量重放到隔离发布分支 `release/ls6-cognition-20260731`，未纳入 LS6 脏工作树数据库或本地备份。
...
- 2026-07-31 LS6 管理看板与认知星图发布候选：销量预警沿用“细分市场 × 新能源、头部前三中位数、25%/80%阈值”规则；06传播势能、07平台阵地和08产品用户之声复用车型无关的公共图表与同口径产品评价数据。认知星图按实际NSR动态扩展坐标域，属性标签、折线与引导线约束在所属象限内；缺失或非法数值保留为待复核，不补0。主线现有企业空间监测关系、政策能源契约和蓝绿发布防护保持优先，不重新引入销量文件兜底名单。应用与全部客户静态资源已统一换版为 `beta-1.03-20260731-ls6-cognition-quadrant-1`；本候选仍需完整门禁、GitHub合并、ECS部署和生产双端验收后才能写为上线。
