...
- 状态包版本：`v2.43`
- 最后核验时间：`2026-07-31`（Asia/Shanghai，云端在线安全发布防护候选）
- Git应用基线：政策自动审核生产闭环提交 `c8839a3` 与抖音逐视频证据链恢复及刷新回显提交 `f9a61db` 已纳入 GitHub 主线；云端在线安全发布防护仍须在提交和生产验收后补记最终提交与部署标记。
- 当前分支：发布源位于隔离分支 `codex/safe-online-deploy-20260731`；现有LS6脏工作树及其数据库不纳入发布归档。
...
- 2026-07-31 抖音逐视频证据链恢复及刷新回显发布候选：逐视频分析现在安全归一化 `douyin.com/video/{id}` 与 `iesdouyin.com/share/video/{id}`，并继续核对平台内容 ID；媒体时长契约已统一，超过5分钟的视频进入长音频转写。OCR 只允许对同次返回的顶层数组做确定性结构包装；任意私网、保留地址仍失败关闭，仅精确允许官方异步转写结果域名在本机代理环境下的合成地址映射。真实智己LS6视频 `7660088055264546082` 在隔离数据库中取得1段转写、12个带时间点的视觉证据与12条评论，证据覆盖为 `full`；初次三路分析有一路因未引用当前视频证据被拒绝，随后只重试失败的第2路，冻结证据指纹未变化，最终交叉验证为 `majority_aligned`。生产真实智己L6视频 `7667383336609261947` 取得1段转写、12个关键画面、24条画面文字及4条评论，三路均完成；因核心判断冲突，系统如实停在 `manual_required`。客户页会显式显示“人工复核前不可直接发布”并禁用通用重跑按钮，同时区分视频内容未读完整、三路尚未启动和单路失败。车型热榜最新结果接口会按当前榜单内容恢复同企业、同版本、同视频的最近逐视频任务，页面刷新或服务重启后不再退回“未分析”按钮，也不会跨车型或跨企业借用任务。主提交 `bca782e` 的完整门禁、六服务部署、核心文件哈希、线上真实任务及两库零漂移已经核验；刷新回显补丁仍须完成完整门禁、提交、部署与双尺寸页面验收。
