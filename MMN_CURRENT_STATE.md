...
- 当前生产分支基线：`codex/social-evidence-v2`；生产应用版本为 `beta-1.03-20260724-security-p1-1`。本地研发分支为 `codex/douyin-video-creation-20260724`，候选版本为 `beta-1.03-20260724-douyin-video-creation-1`，尚未提交、推送或部署。
...
- 2026-07-24 抖音单视频创作转化为本地研发状态：排行榜已验证视频可进入创作简报并生成三条原创方向；新增“爆款内容拆解库”管理外部视频入口、搜索、状态筛选、收藏、结构对比与完整脚本衔接。该能力只迁移内容结构和传播机制，不把热度写成产品或销量因果；证据不足、三路未形成可发布结论时禁止生成。当前仅在隔离工作树和临时 SQLite 验收，未提交、未推送、未部署，生产状态不变。
