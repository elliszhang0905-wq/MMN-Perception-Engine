...
- Git应用基线：生产应用提交为 `d8bf3c9eb5aacc06fb4346ebae16c1bb7c50ac33`；GitHub `main`、发布分支与 ECS `.deployed-git-commit` 已一致。
- 当前分支：`codex/social-evidence-v2`；应用版本为 `beta-1.03-20260724-static-boundary-1`。
- 当前生产状态：竞品格局已改为服务器保存完整正反向关系、浏览器只保存轻量筛选上下文；旧版大缓存仅在服务器恢复成功后迁移，项目快照不再重复携带关系明细，服务器资产按分页全量恢复。决策驾驶舱按“车型 × 平台 × 来源”选择最新完整权威快照，同一张图的排名、占比、趋势与周期不得跨来源拼接；更新但缺少占比的排名文件只提示新鲜度，不覆盖完整趋势。懂车帝周榜图片先进入可编辑预览，只有人工确认平台、本品车型、自然周和逐行排名后才原子写入正式竞品资产；Linux 采用稀疏表格识别和按列聚类，平台字样未可靠识别时强制人工确认原图平台。生产同一张真实图片已识别 10 行与连续自然周，未确认时按钮禁用、确认后启用。社媒趋势补充可追溯竞品召回、相关/热门/风险三类证据池和车型采集漏斗；查询完成不宣称平台全库采尽。
