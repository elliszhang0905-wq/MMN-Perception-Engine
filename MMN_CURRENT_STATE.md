...
- Git应用基线：生产应用提交为 `998aea8f0fec684c380c49b0d43378a8fc1368d6`；GitHub `main`、发布分支与 ECS 运行包已同步。
- 当前生产分支基线：`codex/social-evidence-v2`；生产应用版本为 `beta-1.03-20260724-security-p1-1`。抖音单视频创作分支 `codex/douyin-video-creation-20260724` 已推送远端，本地 8765 运行版本为 `beta-1.03-20260724-douyin-video-creation-1`；ECS 生产未部署。
...
- 2026-07-24 抖音单视频创作转化完成本地发布：排行榜已验证视频可进入创作简报并生成三条原创方向；新增“爆款内容拆解库”管理外部视频入口、搜索、状态筛选、收藏、结构对比与完整脚本衔接。该能力只迁移内容结构和传播机制，不把热度写成产品或销量因果；证据不足、三路未形成可发布结论时禁止生成。功能提交 `f84cadb` 已推送远端分支并从该隔离工作树部署本地 8765，生产状态不变。
...
| 类型 | 当前模块 | 状态说明 |
| --- | --- | --- |
| 已实现并可持久化 | 项目快照、Learning、Workspace、BF 工厂、社媒快照、抖音热点实体复核、按需视频洞察与内容防线、单视频创作方向资产（本地研发）、达人蒸馏、内容能力资产、政策记录与复核、Eval 人工复核 | 持久化在 SQLite、JSON/JSONL 或资产文件中；具体表和路径见下节。单视频创作方向尚未进入生产。 |
...
| 抖音单视频创作方向（本地研发，未生产） | 已验证逐视频洞察或人工粘贴的具体抖音视频链接 → 品牌/车型/人群/任务/利益点/风格简报 → 草稿/复核/定稿 → 三条原创方向 → 选择达人方法生成完整脚本 | 仅手动触发；按组织、版本、洞察、简报指纹和提示词/结构版本幂等复用。必须保留当前视频证据引用，禁止复制原文案、人物身份或镜头组合，禁止把创意简报当产品事实。支持搜索、状态筛选、收藏、最多三项结构对比；证据不足或未验证洞察不可生成 | `douyin_video_creation.py`、`douyin_video_insights.py`、`creator_script_generation.py`、`server.py`、`douyin-hot-demo.js`、`douyin-hot-demo.css` |
...
- 2026-07-24 抖音单视频创作转化本地验收：新增创作方向与逐视频洞察专项合计 `25/25` 通过，抖音榜单、内容防线、脚本生成和本功能扩大回归 `73/73` 通过，覆盖三方向结构合同、当前证据引用、未验证洞察拦截、组织/版本隔离、简报指纹幂等、失败重试、旧表增量升级、收藏持久化、外部具体视频入口和选定方向向完整脚本传递完整来源上下文。隔离服务使用临时 SQLite 和端口 `8879`，真实页面完成“排行榜视频 → 完整洞察 → 创作简报 → 幂等命中 → 爆款内容拆解库 → 三方向展示”，并验证收藏落库、搜索空结果恢复和非法外链拒绝；1440px与390px均完成视觉检查。仓库级发布门禁的语法和前端合同通过，后端189项阶段因隔离工作树未携带品牌穿透基准快照表与外部社媒服务密钥产生14项环境错误；未复制业务库、未注入或输出密钥。脚本按钮的后端衔接由测试桩验证，本轮未调用外部模型评估真实脚本文本质量。功能提交 `f84cadb` 已推送远端分支并部署本地 8765；部署后真实页面确认“榜单发现”和“爆款内容拆解库”在1440px与390px均可用，相关GET接口为200且控制台错误为0。页面懒刷新触碰的5条车型身份记录已按主键从部署前备份精确恢复；原77张表逐表逻辑内容不变，只新增0行的 `douyin_video_creation_plans` 表，`quick_check=ok`。ECS生产未部署。
