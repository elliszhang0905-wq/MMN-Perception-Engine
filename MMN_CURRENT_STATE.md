...
- 2026-08-10 周度车市扫描月末格式本地发布候选：官网最新《车市扫描(20260727-0731)》省略同比月份并将“新能源零售渗透率”改写为“新能源市场零售渗透率”，原解析器因此失败；月末周截至7月31日而非周日8月2日，原新鲜度校验也会阻止发布。候选提交 `f4e05823a132da14a78412ef93aaacb28862ffa6` 兼容两类同比措辞、两类渗透率措辞和同一完成周内的月末截断；若线上错过定时窗口，只在官网最新一期晚于已保存快照时补录一次，同一期再次运行继续保持待发布。浏览器门禁隔离启动期BF项目、其空文档列表和产品评价同步，不改变隔离数据库，并要求84张表前后逻辑零漂移。43项定向Python测试、管理看板UI契约、真实官网抓取、临时数据目录刷新接口及1440px/390px浏览器验收通过；生产尚未部署，线上仍为 `latest_parse_failed`。
