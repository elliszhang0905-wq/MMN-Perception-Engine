...
- Git应用基线：应用运行提交为 `889b63167604b06792829eddebabb4bc01f5e6f7`；已同步 GitHub 发布分支与 `main`，本地 8765 和 ECS 部署标记一致。
- 当前分支：`codex/social-evidence-v2`；GitHub `main` 与发布分支均指向应用运行提交，版本标签为 `beta-1.03-20260723-cockpit-social-brand-1`。
- 本地发布候选：`beta-1.03-20260723-vertical-social-reliability-1`。在此前状态恢复修复基础上，竞品格局已改为服务器保存完整正反向关系、浏览器只保存轻量筛选上下文；旧版大缓存仅在服务器恢复成功后迁移，项目快照不再重复携带关系明细，服务器资产按分页全量恢复。决策驾驶舱的竞争格局进一步改为按“车型 × 平台 × 来源”选择最新完整权威快照，同一张图的排名、占比、趋势与周期不得跨来源拼接；更新但缺少占比的排名文件只提示新鲜度，不覆盖完整趋势。导入后保持用户当前页面，并在权威快照变化时使卖点建议、选题规划和策略资料包失效，等待基于新证据重算。本轮另增加懂车帝周榜图片导入：图片先进入可编辑预览，只有人工确认本品车型、自然周和逐行排名后才原子写入正式竞品资产；确认条件不足时显示具体缺口和明确禁用态。社媒趋势补充经过复核的竞品表达、可追溯召回方式、全部相关/热门/风险三类证据池和车型采集漏斗；查询完成不再宣称平台全库采尽。已在隔离数据库与桌面/390px完成真实流程验证；正式提交、推送和 ECS 部署进入本轮发布流程。
- 基线说明：本轮发布版本为 `beta-1.03-20260723-cockpit-social-brand-1`。公开社媒证据 V2 代码随包交付，但生产开关保持关闭且独立 Worker profile 不启动；既有社媒趋势、周度刷新和决策链路不改变业务口径。
- 当前未提交内容包括本地状态恢复与竞品格局持久化修复代码、测试；`data/` 下本地备份/状态、日志、截图、`output/` 与 `tmp/` 不进入本次 GitHub 或 ECS 发布归档。
...
| 公开社媒证据 V2（随包交付、默认关闭） | 版本化 QueryPlan → 持久化队列/独立 Worker → 原始响应归档 → CanonicalContent → 两个独立 Evidence Mart | 组织/版本/项目隔离；时间窗、排除词、平台与来源角色配额；任务/企业日预算；跨任务缓存；重启恢复；原生指标不跨平台相加；传播证据不外推市场需求、购买意愿或成交因果。Compose Worker 仅在 `social-evidence-v2` profile 下启动 | `social_evidence.py`、`scripts/run_social_evidence_worker.py`、`docker-compose.yml`、`server.py`、`app.js` |
