...
- Git应用基线：抖音逐视频证据链恢复发布候选版本为 `beta-1.03-20260730-douyin-video-evidence-chain-2`；GitHub提交、ECS标记与最终哈希须在生产收口后补记。
...
- 2026-07-30 政策车型自动审核发布候选：政策页与管理看板的本品清单改为读取当前企业空间内已核验的销量预警监测车型档案，所选车型、区域或购车情景变化后自动发起只读预览审核。服务端不信任客户端提交的价格、动力和车身档案；未知车型即使伪造完整参数也返回 `vehicle_profile_incomplete`，权益及政策后价格保持空值且不调用模型。竞品动力类型从各车型原始销量证据逐车传递；多动力车型仅匹配覆盖全部已声明动力形式的政策范围。三方独立审核必须引用共同已审核证据并在政策判断、策略方向和最低置信度上通过门禁，之后由最终归纳环节只选择一份已通过门禁的原始审核，不得重写人群、行动、停止条件或不确定性；单路临时失败只重试失败路一次，仍失败则保持未完成或人工复核。客户界面只显示中性的MMN交叉验证状态。隔离本地真实LS6上海置换情景匹配2项规则，条件权益上限23,403元、条件满足后参考价166,497元；真实多方调用已分别验证三方冲突和单路超时均会拒绝发布，最终选择合同由对抗测试覆盖。完整发布门禁已通过，生产提交、部署与零漂移结论仍须在正式验收后补记。
