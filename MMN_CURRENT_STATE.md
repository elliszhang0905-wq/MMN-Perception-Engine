...
- Git应用基线：`b4e75cf2a2d64cdf98f9e8f627639d346fdd6fe9`；该提交是本地与 ECS 实际运行代码，发布交接文档由后续 docs-only 收口提交记录。
- 当前分支：`codex/social-evidence-v2`；应用基线已快进同步 GitHub `main`，发布分支与 `main` 在文档收口后继续保持一致。
- 生成时工作树状态：发布范围 tracked clean；另有并行中的品牌传播穿透改动 `server.py`、`social_trends.py`、`brand_penetration_analysis.py` 与对应测试，因其在本次应用部署后才进入工作树，未混入本次提交、GitHub、ECS 或本地运行进程。`data/`、日志、截图、`output/`、`tmp/`、用户文档和编辑器锁文件继续作为运行产物排除。
- 基线说明：本地与 ECS 当前运行 `beta-1.03-20260723-social-intelligence-1`。公开社媒证据 V2 代码随包交付，但生产开关保持关闭且独立 Worker profile 不启动；既有社媒趋势、周度刷新和决策链路已按新版本升级。
- 未提交内容包括上述并行品牌传播穿透工作，以及 `data/` 下本地备份/状态、日志、截图、`output/` 与 `tmp/`；这些内容不进入本次 GitHub 或 ECS 发布归档。
...
| 决策驾驶舱子项 | `brandpenetration` | 品牌传播穿透与周度策略视图 | 旧快照链路继续保留；本地新增独立 `brand-penetration-mart-v2`，只表达品牌关联、来源角色和竞品传播观察。V2 默认关闭，尚未生产部署。`app.js`、`demo-brand-weekly-radar.html`、`social_evidence.py`、`server.py` |
| 决策驾驶舱子项 | `socialtrends` | 社媒变化、用户语言、竞品占位与证据明细 | 旧任务/导入/快照继续保留；本地新增 `social-trend-mart-v2`、查询计划、覆盖与缺口显示。V2 默认关闭，服务不可用或证据不足时不得视作实时事实。`app.js`、`social_evidence.py`、`server.py` |
