...
- 2026-07-31 云端在线安全发布防护候选：`scripts/deploy.sh` 不再先执行整套容器停止再构建镜像。发布过程新增互斥锁、4GB默认磁盘门槛、低空间时仅清理无用构建缓存、在线数据备份、上一运行镜像固定回滚标签、独立候选镜像构建与编译烟雾检查、构建超时、健康门控原地切换和失败自动回退；构建失败或磁盘不足时旧版本继续在线。发布脚本聚焦测试41项通过，受控版本数据资产的完整发布门禁通过；当前本地运行数据目录另有既存监测车型口径不一致（预期含智己LS6，实际名单缺失），同样可在未修改的`origin/main`复现，本轮不改写该业务数据。当前改造消除镜像构建期间的分钟级502；单机容器替换与数据读取服务重启仍可能有秒级连接重试，严格零丢包需双实例蓝绿发布。
