...
- Git应用基线：`5e6a19da6a7379a9fa789c3f9de5745a9999c9af`；发布收口文档与生产门禁兼容修复待形成最终提交。
- 当前分支：`codex/social-evidence-v2`；应用提交已快进同步 GitHub `main`，收口提交完成后继续同步该分支与 `main`。
- 生成时工作树状态：`dirty`（仅发布收口文档、发布日期和生产门禁兼容修复待提交；`data/`、日志、截图、`output/`、`tmp/` 和文档编辑器锁文件继续作为本地运行产物排除）。
...
- 2026-07-23：修复社媒趋势中心把本品重复作为竞品采集、聚合和展示，以及证据表默认只显示本品的问题。前后端现在使用统一车型身份键剔除本品和重复竞品，历史快照回读只读去重且不改写数据库；原始内容下钻默认展示全部已选车型并取消前端 30 条静默截断。仅本地验证，未提交、未推送、未部署。
- 2026-07-23：本地新增 7/30 天公开社媒时间窗与决策驾驶舱 NSR 真实标签验证。时间窗按上海自然日冻结，页数、候选、请求和成本仅作安全上限；抖音、小红书、微博分别记录覆盖与停止原因。车型与属性专属词必须同时命中，正负词只判断立场；裁决不改写 NSR，且只有带原文证据的人工确认支持可进入以后新建的决策快照。默认关闭，未提交、未推送、未部署。
- 2026-07-23：社媒趋势中心由固定页数热门样本改为车型别名逐词持续分页，抖音续传 cursor/search_id/backtrace；新增标题、描述、话题和命中字段，跨别名按平台内容 ID 去重。相关内容量、正文情感与风险改用无热度门槛的相关池，热门门槛只控制排行榜；删除样本量“置信度”，显示采集完整性和分析覆盖率。三路独立审阅在 KPI 后发布唯一中性本竞品洞察，采集部分或任一路不完整时条件发布/暂缓发布。仅本地验证，未提交、未推送、未部署。
...
- 2026-07-23 ECS 首轮生产发布：应用提交 `5e6a19da6a7379a9fa789c3f9de5745a9999c9af` 已同步 GitHub 分支与 `main`，服务器部署标记一致；六个核心服务运行，健康接口返回 `beta-1.03-20260723-social-intelligence-1`，公开社媒证据 V2 与独立 Worker 保持关闭。部署前源码、环境与完整数据备份已经保存；部署前后使用同一 SQLite 备份算法比较76张表，既有表行数和逐行指纹变化均为0，`quick_check=ok`。生产浏览器门禁发现镜像仅安装 `playwright-core` 而脚本只引用 `playwright`，现已加入兼容加载并修正发布日期为2026-07-23；最终浏览器验收、收口提交和二次镜像同步完成后在 `release.md` 与交接文件登记。
- 2026-07-23 社媒趋势车型互斥与默认展示修复新鲜验证：项目依赖运行时完整 Python 回归 `558/558` 通过，社媒风险弹层、竞品选择器和本品正向基准 3 个 Node UI 合同通过；本地真实接口对既有奥迪E7X快照返回本品、奔驰GLC EV、问界M7三台唯一车型，15 条证据全部唯一且不存在本品竞品卡，历史快照 JSON 未改写。真实浏览器把本品改为已选竞品 Model Y 后，该车型立即从竞品列表移除；刷新后本品仍不在竞品列表。390px 下页面横向溢出为 0。未触发新的外部采集，未提交、未推送、未部署。
