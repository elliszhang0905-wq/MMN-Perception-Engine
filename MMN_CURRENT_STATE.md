...
- Git应用基线：`df8f220`（当前生产基线）；`beta-1.03-20260723-social-intelligence-1` 正在执行本地发布门禁与正式提交。
- 当前分支：`codex/social-evidence-v2`（今日 MMN 更新的发布候选分支；待提交后快进同步 `main`）。
- 生成时工作树状态：`dirty`（发布候选源码、测试与文档已经通过完整门禁；`data/`、日志、截图、`output/`、`tmp/` 和文档编辑器锁文件继续作为本地运行产物排除）。
- 基线说明：当前生产仍运行 `beta-1.03-20260722-product-evaluation-partial-1`；发布候选版本为 `beta-1.03-20260723-social-intelligence-1`。公开社媒证据 V2 代码随包交付，但生产开关保持关闭且独立 Worker profile 不启动；既有社媒趋势、周度刷新和决策链路按新版本升级。
