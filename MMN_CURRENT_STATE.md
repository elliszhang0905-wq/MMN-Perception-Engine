...
- Git应用基线：`df8f220`（产品评价部分汇总分层导入；已推送 GitHub `main` 并部署）。
- 当前分支：`codex/social-evidence-v2`（本次发布提交单独推送 `main`；该分支其余未完成社媒证据改动未进入发布）。
- 生成时工作树状态：`dirty`（存在与本次发布无关的社媒证据开发文件及既有未跟踪运行产物；本次发布归档只包含 `df8f220`）。
- 基线说明：生产已运行 `beta-1.03-20260722-product-evaluation-partial-1`。产品评价汇总表按指标层级接收：整体声量、互动量和完整全网 NSR 可独立登记；缺少属性 NSR 时不再整表拒绝，属性机会地图保持明确缺口且不计算、不推断。生产目录现含智己L6、奥迪E7X和启境GT7三条正式数据。
