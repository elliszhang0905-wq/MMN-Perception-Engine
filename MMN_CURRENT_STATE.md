...
- 状态包版本：`v2.45`
- 最后核验时间：`2026-07-31`（Asia/Shanghai，政策车型能源契约发布候选）
- Git应用基线：当前政策演示修复候选基于 GitHub `main` 的 `5f57059`，保留抖音逐视频证据链、政策车型动力契约与云端在线安全发布防护，版本目标为 `beta-1.03-20260731-policy-demo-ready-1`；最终 GitHub 提交、ECS 标记与归档哈希须在生产收口后补记。
- 当前分支：发布源位于隔离分支 `fix/policy-energy-contract-20260731`；现有 LS6 脏工作树及其数据库不纳入发布归档。
