...
- 2026-07-30 决策驾驶舱车型内容热度证据候选：在 `03 竞品认知` 内、竞品趋势之后和卖点决策之前提供独立车型输入、7/14/30天与Top 10/20/50。单车型模式不修改驾驶舱本品/竞品配置，服务端强制清空竞品与属性词；搜索按游标受控分页，默认上限为每查询10页、全轮30次请求和300条候选。车型关联或播放量未核验的内容不进入正式榜，缺失播放量不补0；正式榜按播放量、发布时间和平台内容标识稳定排序。达到页数、请求或候选上限时只标记部分覆盖，不宣称平台全库。相关Python回归94项、JavaScript合同、语法与差异检查已通过；1440px和390px本地页面无横向溢出且控制台错误为0。当前仍是发布候选，生产真实运行、数据对比和部署闭环待完成。
