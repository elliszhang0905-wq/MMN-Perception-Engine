...
- Git应用基线：政策能源契约运行提交 `5f57059cac880cb1e6174a17990f906c5f01e4ca` 已进入 GitHub `main` 并部署；其直接后继应用提交 `3e19c3aa06048c636b313529819b46e1d3a9ecc8` 保留能源契约并补齐演示入口，当前生产版本为 `beta-1.03-20260731-policy-demo-ready-1`。
...
- 2026-07-31 LS6 管理看板与认知星图发布候选：销量预警沿用“细分市场 × 新能源、头部前三中位数、25%/80%阈值”规则；06传播势能、07平台阵地和08产品用户之声复用车型无关的公共图表与同口径产品评价数据。认知星图按实际NSR动态扩展坐标域，属性标签、折线与引导线约束在所属象限内；缺失或非法数值保留为待复核，不补0。主线现有企业空间监测关系、政策能源契约和蓝绿发布防护保持优先，不重新引入销量文件兜底名单。本候选已形成隔离提交，仍需完整门禁、GitHub合并、ECS部署和生产双端验收后才能写为上线。
