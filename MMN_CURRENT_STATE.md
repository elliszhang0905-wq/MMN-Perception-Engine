...
- Git应用基线：应用提交 `909a4a48370d15510851e01e395dc12b1ba5de01` 已同步功能分支、GitHub `main`、标签 `beta-1.03-20260729-douyin-vehicle-radar-1`、本地8765与ECS；服务器 `.deployed-git-commit`、健康版本和核心文件哈希一致。GitHub `main` 可在本状态包提交后仅领先纯文档闭环，不代表应用代码漂移。
- 当前分支：生产仍运行 `codex/douyin-radar-metrics-20260729` 对应的应用提交；本轮候选位于隔离分支 `codex/mmn-dashboard-dy-rank-20260730`，未通过完整门禁、GitHub提交、生产备份、真实TikHub运行和双端验收前不得写成已发布。
...
- 2026-07-29 全板块收口候选：逐项审计代码存在、生产部署、生产启用和真实验收，不以“代码存在”等同“已启用”。公开社媒证据 V2、NSR 公开讨论验证、封面 Prompt 和独立 Worker 继续受控关闭；政策智能保持 MVP 与人工复核，内容策略保留初稿/降级标签，BF 保持 P0，Eval 仅作离线发布门禁，正式 Know-how 仍需真实行动结果、重复验证和人工批准，行动预算继续隐藏。
...
- 2026-07-29 TikHub 达人链路加固候选：作品资产唯一键升级为“企业空间 × 平台 × 平台作品 ID”，达人详情、任务读取/暂停/重试、观点判断和方法资产均按企业空间失败关闭；旧 SQLite 资产按达人所属企业原位迁移，无法安全归属的方法资产标为 `legacy-unscoped`，不向任一客户空间透出。同一企业、主页、范围、样本数和确认达人名的活动任务幂等复用；单次适配器最多 30 次外部请求尝试，重试计入上限。
