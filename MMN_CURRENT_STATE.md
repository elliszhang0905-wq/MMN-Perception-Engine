...
- 状态包版本：`v2.42`
- 最后核验时间：`2026-07-31`（Asia/Shanghai，抖音逐视频证据链恢复及刷新回显补丁发布候选）
- Git应用基线：政策自动审核生产闭环提交 `c8839a3` 已纳入当前基线；抖音逐视频证据链恢复主提交 `bca782e` 已推送 GitHub 主线并部署 ECS，运行版本为 `beta-1.03-20260730-douyin-video-evidence-chain-3`。刷新回显补丁仍须在完整门禁、提交和生产验收后补记最终提交。
- 当前分支：发布源位于隔离分支 `fix/douyin-video-evidence-chain-30260730`；现有LS6脏工作树及其数据库不纳入发布归档。
