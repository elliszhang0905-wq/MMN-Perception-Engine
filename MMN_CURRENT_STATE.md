...
- Git commit：`188f6bfc4df5cdbcd02db7100ec15af87b8ab033`（短 hash `188f6bf`）。
- 当前分支：`main`
- 生成时工作树状态：`dirty`（`beta-1.03-20260722-product-evaluation-catalog-2` 递归保护修复待提交；其他未跟踪运行产物保持不动）
- 基线说明：当前 `main` 已包含 `beta-1.03-20260722-decision-closure-1` 决策闭环发布；本轮将标准驾驶舱产品评价数据改为按组织、版本和源车型登记与恢复，切换车型时有数据即显示对应看板、无数据才进入缺口态。新增一张目录表和读写接口，不修改原始导入文件或既有业务表记录。
...
- 2026-07-22：将产品评价数据从“页面内只记住当前包”改为按组织、版本和源车型隔离的浏览器缓存与服务器目录；应用启动、版本切换和重新登录会恢复已登记数据，内置智己L6数据会重新注册，E7X及后续导入包会持久化，同车型新包替换旧包，空状态不能覆盖已有车型数据。新增 `product_evaluation_datasets` 表及鉴权目录接口，选择车型时只绑定该车型所属数据包，真正缺数的车型继续明确显示缺口。发布候选已通过完整门禁，待提交、推送和生产验收。
