...
- Git应用基线：生产应用提交为 `f3b35738e2327026e65fe1e6d944325c54210844`；GitHub `main`、发布分支、最终版本标签与 ECS `.deployed-git-commit` 已一致。
...
- 2026-07-24：静态资源边界加固已完成本地与隔离验收，应用层改为显式公开资源白名单，Nginx增加敏感路径独立阻断，GET与HEAD及编码绕过均纳入真实HTTP测试；本次不迁移数据库、不修改业务接口或看板计算。生产发布前基线已备份并生成77张业务表逐表逻辑指纹，当前处于正式发布执行中。
