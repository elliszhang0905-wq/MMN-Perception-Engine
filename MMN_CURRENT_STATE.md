...
- Git应用基线：当前 `HEAD` 为 `c1b04b7bda488517282efd647b17cbd43e3bc363`；本文档对应的三中心中型迭代已完成本地发布候选验收，尚待形成应用提交并同步 GitHub/ECS。
- 当前分支：`codex/social-evidence-v2`；本轮发布前 GitHub `main` 基线为 `c1b04b7bda488517282efd647b17cbd43e3bc363`。
- 生成时工作树状态：待发布文件仅包含品牌穿透三路结论、社媒快照恢复/空态/口径对齐、版本与缓存标识、回归测试和发布文档。`data/`、日志、截图、`output/`、`tmp/`、用户文档和既有未跟踪备份继续排除，不进入 GitHub 或 ECS 归档。
...
- 当前未提交发布候选包括上述三中心代码、测试与档案；`data/` 下本地备份/状态、日志、截图、`output/` 与 `tmp/` 不进入本次 GitHub 或 ECS 发布归档。
...
- 2026-07-23 三中心中型系统迭代发布候选验证：使用依赖完整运行时完成全量 Python `586/586`；全部 Node 契约、状态门禁和研发档案检查通过；仓库发布门禁完成 161 项后端/契约验证及全表面真实浏览器回归，覆盖 19 个客户入口、8 个管理视图、E7X 数据绑定、无数据清空、社媒趋势分析/导入/自定义日期/风险与来源下钻，桌面及 390px 均为 `failed=[]`、`runtimeErrors=[]`、`failedResponses=[]`。品牌穿透独立真实流程同时验证 6 张逐品牌卡、5 张逐竞品卡、筛选下钻和刷新恢复。验收造成的商业库变更已用发布前逐文件备份完整恢复，商业库与社媒证据库 SHA-256 均与备份一致且 `quick_check=ok`。
