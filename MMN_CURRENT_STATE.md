...
- 状态包版本：`v2.53`
- 最后核验时间：`2026-08-10`（Asia/Shanghai，线索看板纵向业务模板本地候选验收）
- Git应用基线：生产运行基线仍为 `beta-1.03-20260810-weekly-market-scan-1`；本次待发布候选为 `beta-1.03-20260810-lead-import-vertical-template-1`，最终提交、归档和部署证据在发布完成后回填。
- 当前分支：生产应用基线为 GitHub `main`；已审计增量同时保留在 `release/weekly-market-scan-20260810`。原主工作树脏改动、本地备份和未审计实验均未进入 GitHub 或 ECS 发布归档。
...
- 2026-08-10 线索看板纵向业务模板发布候选：兼容“分阶段转化”区域按阶段纵向列出线索目标、线索达成、订单目标和订单达成的既有 xlsx，车型继续由工作表名经统一车型归一化确定，不增加 E7X 专属分支。文件内最后阶段确定为进行中，其余阶段确定为已完成，并在来源元数据中记录该状态口径；缺车型、缺指标或重复指标均失败关闭，非法更新不会覆盖旧数据。真实 0716 文件导入后恢复 4 阶段旧看板，真实 0806 文件连续导入后形成 5 阶段，当前 8 月线索 40,440 / 377,143、订单 272 / 3,300；隔离端口 8876 页面刷新后持久恢复，两个上传请求均为201且控制台错误为0。完整发布门禁使用仓库版本化数据文件和业务SQLite只读备份通过：全量Python、全部JavaScript合同、19个客户入口、8个管理视图、1440px/390px均无失败，启动、浏览器与源库86张表逻辑零漂移。当前仍为隔离工作树候选，尚未提交、推送、切换8765或部署ECS。
