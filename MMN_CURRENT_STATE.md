...
- Git应用基线：发布前生产应用提交为 `7cee9d13cdfc956efa1792fdedcc2085a1519769`；发布前 GitHub `main` 为 `61b07bfd982c025a1a0e2be58ff6df19c2bb849b`，后者只比生产运行包多发布归档文档提交。
- 当前分支：干净生产发布分支 `release/mmn-audit-legacy-20260727`；已将审计实现 `6a39eb706af5ededf19296da1b3704b2e401b6c5` 与归档收口 `079321dd3b49b5759a8730337045b3b32af1e517` 合入主线候选，待发布版本及全局静态资源缓存令牌均为 `beta-1.03-20260727-audit-hardening-1`，尚未推送或部署。
...
- 2026-07-27 历史独立审计本地闭环：原候选与生产主线存在分叉，候选缺少生产已发布的认证密钥隔离、可信代理、调度签名和原始上传读取边界；已在隔离分支合并生产 `7cee9d1`，新增三项回归保护，并修复周度摘要测试的日期漂移和社媒测试对真实密钥的隐式依赖。发布门禁现于任何测试前建立隔离环境，强制使用独立端口、临时数据/输出/缓存目录、只读源库备份和每个 Python 模块独立数据库，Eval与社媒证据默认路径均遵循隔离数据根，拒绝复用已有服务；完整 Python 651项按83个文件独立通过，33个 JavaScript 测试文件和27项真实 HTTP 边界通过，状态包检查已纳入门禁。真实业务门禁覆盖数据优先路径、19个客户入口、8个管理视图和1440px/390px全表面，`failed=[]`、`runtimeErrors=[]`、`failedResponses=[]`；源库复制、服务初始化、浏览器验收三层79张表逻辑变化均为0，门禁前后工作树指纹一致。依赖审计已清除高危和低危项，保留1项仅涉及未使用 XML 构建接口的中危报告；原 `data/` 67个文件 SHA-256 全部一致，`quick_check=ok`。本轮不推送、不部署。
