...
- Git应用基线：本次发布代码基于生产主线 `0603810`；最终应用提交、GitHub 与 ECS 状态待生产验收后回填。
...
- 2026-07-25 抖音视频第三路独立分析安全重试进入发布：修复免费额度权限恢复后的错误映射、冻结证据包复用、仅失败槽位重试与独立超时配置；客户界面继续隐藏供应商和模型名称。最终提交、生产业务流与数据零漂移证据待部署验收后回填。
