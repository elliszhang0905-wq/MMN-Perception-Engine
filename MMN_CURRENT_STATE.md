...
- 状态包版本：`v2.46`
- 最后核验时间：`2026-07-31`（Asia/Shanghai，政策演示入口与交叉复核生产闭环）
- Git应用基线：应用提交 `3e19c3aa06048c636b313529819b46e1d3a9ecc8` 已进入 GitHub `main` 并以归档 SHA-256 `ca43198bed8f1d04a15c168e2fe776898f2c665fd46439570926c00358667d8f` 部署ECS，生产版本为 `beta-1.03-20260731-policy-demo-ready-1`。
- 当前分支：发布源位于隔离分支 `fix/policy-demo-ready-20260731`；现有 LS6 脏工作树及其数据库不纳入发布归档。
...
- 2026-07-31 政策车型能源契约发布候选：政策计算在既有“复合动力拆分、未知档案不补0、竞品逐车使用自身动力事实”门禁上，新增统一能源别名契约，兼容 BEV、EREV、PHEV、HEV、ICE、中文别名及斜杠、加号、逗号、竖线等复合分隔符，并保留原始能源文本、标准代码、解析状态、来源与时间。任何已知类型与未知类型混合的输入整体失败关闭；多动力车系若只有部分动力版本命中政策，返回 `variant_required`，权益及政策后价格保持空值，客户页提示选择具体动力版本，不再显示为真实0。现有生产 `main` 已具备 LS6 上海置换场景2项已审核规则、条件权益上限23,403元和条件满足后参考价166,497元的基础修复；本候选补齐未来命名和部分版本政策的防复发门禁。完整发布门禁、提交、生产部署与零漂移结论须在正式收口后补记。
