...
- 状态包版本：`v2.42`
- 最后核验时间：`2026-07-31`（Asia/Shanghai，抖音逐视频证据链恢复及刷新回显生产闭环）
- Git应用基线：抖音逐视频证据链恢复及刷新回显功能提交 `f9a61db40370c2666faa9f2cc5141cdc539aaaa0` 已推送 GitHub `main` 与发布分支并部署 ECS，运行版本为 `beta-1.03-20260730-douyin-video-evidence-chain-3`。
- 当前分支：发布源位于隔离分支 `fix/douyin-video-evidence-chain-20260730`；现有LS6脏工作树及其数据库未纳入发布归档，本地8765为保护该并行工作而未强制切换。
