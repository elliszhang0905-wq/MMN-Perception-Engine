...
- Git应用基线：当前生产代码基线为 `103cb3757546e0182e4e489d6dd8bdbea1874ecf`，发布核验档案提交为 `b6845a13c27c8c5ed6efbae126fb0f7afee06456`；本轮发布候选已合并该主线。
- 当前分支：`codex/social-evidence-v2`；待本轮完整门禁通过后同步 GitHub 发布分支与 `main`，并发布新版本标签。
- 本地发布候选：`beta-1.03-20260723-vertical-social-reliability-2`。在此前状态恢复修复基础上，竞品格局已改为服务器保存完整正反向关系、浏览器只保存轻量筛选上下文；旧版大缓存仅在服务器恢复成功后迁移，项目快照不再重复携带关系明细，服务器资产按分页全量恢复。决策驾驶舱的竞争格局进一步改为按“车型 × 平台 × 来源”选择最新完整权威快照，同一张图的排名、占比、趋势与周期不得跨来源拼接；更新但缺少占比的排名文件只提示新鲜度，不覆盖完整趋势。导入后保持用户当前页面，并在权威快照变化时使卖点建议、选题规划和策略资料包失效，等待基于新证据重算。本轮另增加懂车帝周榜图片导入：图片先进入可编辑预览，只有人工确认平台、本品车型、自然周和逐行排名后才原子写入正式竞品资产；确认条件不足时显示具体缺口和明确禁用态。生产首轮真实图片门禁发现 Linux 默认 OCR 合并表格单元格且漏读平台标题，补丁版已改为稀疏表格识别、按列聚类，并在平台字样未可靠识别时强制人工确认原图平台。社媒趋势补充经过复核的竞品表达、可追溯召回方式、全部相关/热门/风险三类证据池和车型采集漏斗；查询完成不再宣称平台全库采尽。已完成本地隔离数据库与桌面/390px验证，生产补丁仍须通过同一张真实图片上传后才可收口。
- 基线说明：主线证据路由与封面 Prompt 代码随包保留，但公开社媒证据 V2、封面 Prompt、影子模式和独立 Worker profile 仍默认关闭；在真实插件影子导入与真实三旗舰成本/质量验收前，不得写成已启用。
- 当前未提交内容仅包括待完成的合并/发布闭环档案；`data/` 下本地备份/状态、日志、截图、`output/` 与 `tmp/` 不进入 GitHub 或 ECS 发布归档。
