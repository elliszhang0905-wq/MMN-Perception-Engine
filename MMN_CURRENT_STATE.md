...
- Git应用基线：`0e0f505`（车型产品评价目录及递归保护；发布档案收口提交随后同步）。
- 当前分支：`main`
- 生成时工作树状态：`dirty`（仅发布档案收口待提交；其他未跟踪运行产物保持不动）
- 基线说明：生产已运行 `beta-1.03-20260722-product-evaluation-catalog-2`。标准驾驶舱产品评价数据按组织、版本和源车型登记与恢复，切换车型时有数据即显示对应看板、无数据才进入缺口态；新增一张目录表和读写接口，不修改原始导入文件或既有业务表记录。
