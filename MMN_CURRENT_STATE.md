...
- 2026-07-31 云端在线安全发布防护候选：`scripts/deploy.sh` 不再先执行整套容器停止再构建镜像。发布过程新增互斥锁、4GB默认磁盘门槛、低空间时仅清理无用构建缓存、在线数据备份、上一运行镜像固定回滚标签、被替代悬空旧镜像清理、独立候选镜像构建与编译烟雾检查、构建超时、并行候选实例、Nginx热切换和失败自动回退；Nginx配置改用目录级只读挂载，宿主机更新路由文件后由既有代理进程无重启重载，避免文件级挂载固定旧inode而导致切换未生效。构建失败或磁盘不足时旧版本继续在线，正式容器替换时由健康候选实例承接流量。发布脚本聚焦测试与受控版本数据资产的完整发布门禁通过；当前本地运行数据目录另有既存监测车型口径不一致（预期含智己LS6，实际名单缺失），同样可在未修改的`origin/main`复现，本轮不改写该业务数据。单机蓝绿切换覆盖镜像构建与正式容器替换窗口；服务器、Docker服务或Nginx自身故障仍需多机高可用覆盖。
