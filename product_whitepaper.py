...
def select_product_pages(pages, per_label=2, max_pages=18, max_chars=32000):
    ...
    for label, keywords in PRODUCT_LABEL_KEYWORDS.items():
        ...
        selected.extend(page for _, page in ranked[:per_label])
    selected = list(dict.fromkeys(([1, 2] if 1 in pages else [2]) + selected))[:max_pages]
    output, used = [], 0
...
5. 优先提取带参数、功能机制、配置或可验证结果的事实，最多 12 条。
...
def normalize_capabilities(raw, pages, limit=12):
...
对保留项目必须原样返回 label、claim、quote、page；不允许改写 quote。最多 12 条。
